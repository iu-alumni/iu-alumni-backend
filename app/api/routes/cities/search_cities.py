...
@router.get("/coordinates", response_model=Optional[Coordinates])
async def get_coordinates(
    ...
    """
    result = db.query(City.lat, City.lng).filter(
        and_(
...
async def search_cities(
    ...
    cities = db.query(City.city, City.country, City.lat, City.lng).filter(
        func.lower(City.city).like(func.lower(f"{search_term}%"))
    ).limit(limit).all()
