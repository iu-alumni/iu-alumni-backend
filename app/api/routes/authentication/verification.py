...
from app.schemas.auth import VerifyGraduateRequest, TokenResponse

router = APIRouter()

@router.post("/verify-graduate", response_model=TokenResponse)
def verify_graduate(
    request: VerifyGraduateRequest,
    ...
    if (user.graduation_year != request.graduation_year or 
        user.first_name.lower() != request.first_name.lower()):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
